...
import { ProcessingStepId, createMockProcessingSteps } from "../components/video/processing-steps";
import { TitleOption, TitleSelector } from "../components/video/title-selector";
import { Thumbnail, ThumbnailGallery } from "../components/video/thumbnail-gallery";
...
function VideoDetailComponent() {
            ...
            const mockThumbnails = [
              {
                ...
                prompt: "Professional headshot with blurred background, high contrast",
              ...
              },
              {
                ...
                prompt: "Action shot with dynamic composition and vibrant colors",
                status: "ready" as const
              },
              {
                id: "thumb-4",
                url: "https://picsum.photos/id/13/640/360",
                prompt: "Infographic style with text overlay and minimal design",
                status: "ready" as const
              }
            ];
  ...
  const handleTitleVote = (titleId: string) => {
    setTitleOptions(prev => 
      prev.map(title => 
        title.id === titleId ? { ...title, votes: (title.votes || 0) + 1 } : title
      )
    );
  };
  ...
  const handleRegenerateSingleThumbnail = (thumbnailId: string, newPrompt: string) => {
  ...
  const thumbnailUrl = videoData.thumbnails?.[0]?.url || thumbnails.find(t => t.isSelected)?.url;
  ...
  return (
    <div className="container py-10">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold tracking-tight mt-4">{videoData.title || "Untitled Video"}</h1>
        <div className="flex items-center text-sm text-muted-foreground mt-1">
          <span className="text-primary">#{videoId.slice(0, 6)}</span>
          {videoData.current_stage && (
            <>
              <span className="mx-2">•</span>
              <span className="capitalize">{videoData.current_stage.replace("_", " ")}</span>
            </>
          )}
        </div>
      </div>
        ...
        <TabsContent value="overview" className="animate-in space-y-6">
          <VideoDetail
            ...
            onSaveContent={async (contentType, content) => {
              ...
              console.log(`Saving ${contentType} content:`, content.substring(0, 100) + "...");
