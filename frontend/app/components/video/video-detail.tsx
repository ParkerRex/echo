...
import { ArrowLeft, CheckCircle, Download, ExternalLink, LucideIcon, Loader2 } from "lucide-react";
...
export function VideoDetail({
  ...
  // Active tab
  const [activeContentTab, setActiveContentTab] = useState<string>("transcript");
    ...
    const fetchContent = async () => {
      ...
      switch (activeContentTab) {
        case "transcript":
          ...
          try {
            const content = await tryFetchContent(bucket_name, output_files, "transcript");
        ...
        case "subtitles":
          ...
          try {
            const content = await tryFetchContent(bucket_name, output_files, "subtitles");
        ...
        case "chapters":
          ...
          try {
            const content = await tryFetchContent(bucket_name, output_files, "chapters");
            setChapters(content || "");
          } catch (error) {
            setChaptersError(error instanceof Error ? error.message : "Failed to load chapters");
          } finally {
        ...
        case "shownotes":
          ...
          try {
            const content = await tryFetchContent(bucket_name, output_files, "shownotes");
            setShownotes(content || "");
          } catch (error) {
            setShownotesError(error instanceof Error ? error.message : "Failed to load shownotes");
          } finally {
    ...
    <div className={className}>
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight line-clamp-1">{videoTitle}</h1>
          {youtubeUrl && (
                ...
                <TabsContent value="transcript" className="space-y-4">
                  {isCompleted ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
                      ...
                      <p className="text-xs mt-1">This will be available once processing is complete</p>
                    </div>
                  )}
                </TabsContent>
