...
from video_processor.core.processors.video import process_video_event, process_video
from video_processor.core.models import VideoJob, VideoMetadata, ProcessingStage, ProcessingStatus
...
def test_process_video_event_sets_up_job(
    ...
    with patch("video_processor.core.processors.video.process_video") as mock_process_video:
        # Call the function
...
def test_process_video_downloads_file():
    ...
    with patch("video_processor.core.processors.video.tempfile.TemporaryDirectory") as mock_temp_dir:
        mock_temp_dir_instance = MagicMock()
        mock_temp_dir_instance.__enter__.return_value = "/tmp/test_dir"
        mock_temp_dir.return_value = mock_temp_dir_instance
        
        # Also need to mock get_settings
        with patch("video_processor.core.processors.video.get_settings") as mock_get_settings:
            ...
            mock_storage_service.download_file.assert_called_once_with(
                "test-bucket", "daily-raw/test_video.mp4", "/tmp/test_dir/test_video.mp4"
            )
